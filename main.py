...
#             class_acc[first_cls:]))
#
#         # if ((np.mean(class_iou[:first_cls]) + np.mean(class_iou[first_cls:])) / 2 < (before_iou+after_iou)/2):
#         #     before_iou = np.mean(class_iou[:first_cls])
#         #     after_iou = np.mean(class_iou[first_cls:])
#
#
...
def get_dataset(opts, seed):
    ...
    dataset_dict['train'] = dataset(opts=opts, image_set='train', transform=train_transform,
                                    cil_step=opts.curr_step, seed=seed)
...
def main(opts, seed):
    ...
    #     opts.batch_size = 2
    ###########################################################################
    dataset_dict = get_dataset(opts, seed=seed)
    train_loader = data.DataLoader(
        dataset_dict['train'], batch_size=opts.batch_size, shuffle=True, num_workers=4,
        pin_memory=True, drop_last=True)
    val_loader = data.DataLoader(
    ...
    # print('\n printing max and min index 7777 train label: ')
    # print(dataset_dict['train'][7777][1].size())
    # print(torch.max(dataset_dict['train'][7777][1]))
    # print(torch.min(dataset_dict['train'][7777][1]))
    # print(dataset_dict['train'][7777][1].is_cuda)
    #
    # print('\n printing unique class in train label: ')
    # print(torch.unique(dataset_dict['train'][7777][1]))
    # return
    ##################################################################################
...
if __name__ == '__main__':
    ...
    print('SSUL will be run ' + str(num_runs) + " times")
    ...
    if start_step == 0:
        ...
        for i in range(num_runs):
            for step in range(start_step, total_step):
                ...
                if (step == 10 and opts.dataset == 'ade') or (step == 5 and opts.dataset == 'coco') \
                        or (step == 4 and opts.dataset == 'coco'):
