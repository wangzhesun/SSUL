...
DATASET=voc
TASK=15-1-split1 # [15-1, 10-1, 19-1, 15-5, 5-3, 5-1, 2-1, 2-2] add split to task 15-1
EPOCH=50 #50
BATCH=16 #32
LOSS=bce_loss
LR=0.01
...
START=1
#START=0
MEMORY=100 #100 # [0 (for SSUL), 100 (for SSUL-M)]
...
python main.py --data_root ${DATA_ROOT} --model deeplabv3_resnet101 --gpu_id 0 --crop_val \
               --lr ${LR} --batch_size ${BATCH} --train_epoch ${EPOCH} --loss_type ${LOSS} \
                ...
                --pseudo_thresh ${THRESH} --freeze --bn_freeze --unknown --w_transfer --amp \
                --mem_size ${MEMORY} \
                --ckpt ./checkpoints/deeplabv3_resnet101_voc_15-1-split1_step_0_disjoint.pth \
                --start_step ${START}
